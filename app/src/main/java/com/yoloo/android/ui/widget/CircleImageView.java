...
import com.yoloo.android.ui.util.ThemeUtils;
...
public class CircleImageView extends AppCompatImageView {
    ...
    /**
     * Helper for creating a circle bitmap drawable using the {@link RoundedBitmapDrawable}
     *
     * @param bitmap The bitmap which should be converted to a circle bitmap drawable
     * @return the {@link RoundedBitmapDrawable} containing the bitmap
     */
    public static RoundedBitmapDrawable getCircleBitmapDrawable(Context context, Bitmap bitmap) {
        ...
        // If height and width doesn't equal, make it round.
        if (height != width) {
            drawable.setCornerRadius(Math.max(width, height) / 2);
        } else {
    ...
    public static Bitmap getBitmapFromDrawable(Drawable drawable) {
        ...
        if (drawable instanceof BitmapDrawable) {
            Timber.w("For better performance consider using setImageBitmap() instead!");
    ...
    public void setImageDrawable(Drawable drawable) {
        if (mIsCircleImageEnabled && drawable != null &&
                ...
                && !(drawable instanceof PlaceholderDrawable)) {
     ...
     * <p/>
     * Note: This usually should be the first letter of a name. You can use the helper {@link
     * #retrieveLetter(String)}
