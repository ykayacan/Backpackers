 ...
 */
public class RelativeTimeTextView extends BaselineGridTextView {
    ...
    private boolean mTrimAgoSuffix;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            mTrimAgoSuffix = a.getBoolean(R.styleable.RelativeTimeTextView_trim_ago_suffix, false);
        ...
        try {
            mReferenceTime = Long.parseLong(mText);
    ...
    private void updateTextDisplay() {
        ...
        String relativeTime = getRelativeTimeDisplayString().toString();

        if (mTrimAgoSuffix) {
            if (relativeTime.contains("days")) {
                relativeTime = relativeTime.replace(" days", "d");
            }
            if (relativeTime.contains("hr")) {
                relativeTime = relativeTime.replace(" hr.", "h");
            }
            if (relativeTime.contains("ago")) {
                relativeTime = relativeTime.replace("ago", "");
            }
            if (relativeTime.contains("min")) {
                relativeTime = relativeTime.replace(" min.", "m");
            }
            if (relativeTime.contains("Just Now")) {
                relativeTime = relativeTime.replace("Just Now", "now");
            }
            if (relativeTime.contains("Yesterday")) {
                relativeTime = relativeTime.replace("Yesterday", "1d");
            }
        }
        ...
        /*if (mTrimAgoSuffix) {
            if (relativeTime.contains(". ago")) {
                relativeTime = relativeTime.replace(". ago", "");
            } else if (relativeTime.contains("sa. önce")) {
                relativeTime = relativeTime.replace("sa. önce", "s");
            }
        }*/

        setText(mPrefix + relativeTime + mSuffix);
    }
    ...
    private CharSequence getRelativeTimeDisplayString() {
        ...
        return (difference >= 0 && difference <= DateUtils.MINUTE_IN_MILLIS)
                ...
                : DateUtils.getRelativeTimeSpanString(mReferenceTime, now,
